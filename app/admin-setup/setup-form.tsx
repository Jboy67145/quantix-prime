...
export function AdminSetupForm() {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      if (signInResult.error) {
        if (!signInResult.error.message.toLowerCase().includes('invalid login')) throw new Error(signInResult.error.message)
        const signUpResult = await signUp.email({ email, password })
        if (signUpResult.error) throw new Error(signUpResult.error.message)
        if (!signUpResult.data.session) throw new Error('Check your email to confirm this administrator account, then return here and sign in.')
      }
  ...
  return <section className="auth-card"><p className="eyebrow">Private operations setup</p><h1>Create administrator access</h1><p className="auth-subtitle">Use the dedicated administrator email to create or activate the first Supabase administrator account.</p><form onSubmit={submit}><label>Email address<input type="email" value={email} onChange={(e) => setEmail(e.target.value)} readOnly /></label><label>Password<input type="password" minLength={8} required value={password} onChange={(e) => setPassword(e.target.value)} placeholder="At least 8 characters" /></label>{error && <p className="auth-error" role="alert">{error}</p>}<button className="primary-button full" disabled={pending}>{pending ? 'Creating secure access…' : 'Create administrator account'}</button></form></section>
}
