...
].map(([name, minimumMinor, durationDays, purchaseBonusMinor, returnMinor], index) => ({ id: `00000000-0000-0000-0000-${String(index + 11).padStart(12, '0')}`, name, description: `${name} investment plan`, category: Number(durationDays) <= 5 ? 'DAILY' : Number(durationDays) <= 20 ? 'WEEKLY' : 'MONTHLY', minimumMinor, maximumMinor: minimumMinor, durationDays, purchaseBonusMinor, returnMinor, returnBps: Math.round(Number(returnMinor) * 10000 / Number(minimumMinor)), active: true, displayOrder: index + 1 }))
...
export async function getPublicPlans() {
  ...
  return plans.map((plan: any) => { const profit = Math.round(Number(plan.minimum_minor ?? plan.minimumMinor) * Number(plan.return_bps ?? plan.returnBps ?? 0) / 10000); const returnMinor = Number(plan.return_minor ?? plan.returnMinor ?? profit); return { ...plan, minimumMinor: Number(plan.minimum_minor ?? plan.minimumMinor), maximumMinor: Number(plan.maximum_minor ?? plan.maximumMinor), durationDays: Number(plan.duration_days ?? plan.durationDays), returnMinor, totalEarningsMinor: Number(plan.minimum_minor ?? plan.minimumMinor) + returnMinor, dailyEarningsMinor: Math.round(returnMinor / Number(plan.duration_days ?? plan.durationDays)), totalExpectedPayoutMinor: Number(plan.minimum_minor ?? plan.minimumMinor) + returnMinor + Number(plan.purchase_bonus_minor ?? plan.purchaseBonusMinor ?? 0), purchaseBonusMinor: Number(plan.purchase_bonus_minor ?? plan.purchaseBonusMinor ?? 0), status: 'OPEN' } })
}
