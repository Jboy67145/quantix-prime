...
export async function submitWithdrawal(input: z.input<typeof withdrawalSchema>) {
  ...
  const { data: withdrawal, error } = await supabase.from('quantix_withdrawals').insert({ user_id: userId, amount_minor: data.amountMinor, fee_minor: feeMinor, net_minor: data.amountMinor - feeMinor, payout_account_snapshot: account, status: 'PENDING' }).select().single()
